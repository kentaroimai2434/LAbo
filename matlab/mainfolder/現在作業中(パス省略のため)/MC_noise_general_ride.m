...
%% データ作成
%ノイズあり準備
fileID =fopen('makeyglass_added_uniform_[-0.25 0.25]1200plot.tex','r');
formatspac='%f';
...
MCbase=sum(MCkbase);
%% データ保存
fileID = fopen('LErandam_makey_noise_1200_[-0.25,0.25]SW.tex','a');
fprintf(fileID,'%f\n',ria);
fclose(fileID);
fileID = fopen('MCrandam_makey_noise_ridge_1200_[-0.25,0.25]SW.tex','a');
fprintf(fileID,'%f\n',MC);
fclose(fileID);
fileID = fopen('MCrandam_makey_noise_regression_1200_[-0.25,0.25]SW.tex','a');
fprintf(fileID,'%f\n',MCbase);
fclose(fileID);
%%
end
