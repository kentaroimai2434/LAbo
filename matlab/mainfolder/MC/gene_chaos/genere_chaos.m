%% プログラミング MC  リッジ　一般化　minimum 0入力
clear;
clc;
%% データ作成
%データ設定
...
%{
%マッキーグラス
fileID =fopen('makeyglasstimedate12000plot.tex','r'); %Δt=0.1
formatspac='%f';
...
Win=(rand(resSize,inSize)*0.000002)-0.000001;%[-0.1,0.1]の一様分布(by論文から）
%Win=normrnd(0,exp(-0.5),[1,150]);
%Win=Win';
...
X(:,1)=tanh(Win*E(1,1));
for t=2:1:AL%学習活性化
   X(:,t)=tanh(Win*E(t,1)+W*X(:,t-1));            %内部状態活性化                          
end
...
Xria_true(:,:)=X(:,a+1:a+SL);                   %正しい状態行列を代入
...
for t=1:riaSize-1      %リアプノフ指数の移動
    ...
    xp=tanh(Win*E(a+(t+1))+W*xp);               %摂動を与えられた内部状態ベクトル正規化したもので計算なのでxp
...
%% データ保存
fileID = fopen('LEmakey_1200_SCR_v[1.01]_in[0]_test1.tex','a');
fprintf(fileID,'%f\n',lya);
fclose(fileID);
fileID = fopen('MCmakey_1200_ridge_SCR_v[1.01]_in[0]_test1.tex','a');
fprintf(fileID,'%f\n',MC_ride);
fclose(fileID);
fileID = fopen('MCmakey_1200_regression_SCR_v[1.01]_in[0]_test1.tex','a');
fprintf(fileID,'%f\n',MCbase);
fclose(fileID);
fileID = fopen('MCmakey_1200_RMSE_SCR_v[1.01]_in[0]_test1.tex','a');
fprintf(fileID,'%f\n',rmse);
fclose(fileID);
%% メモ
%plot(1:5000,X(1,1:5000))
%plot(X(1,1:4999),X(1,2:5000))
%end
%end 