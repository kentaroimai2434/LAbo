%% プログラミング MC ノイズマッキーグラス リッジ　一般化
clear;
clc;
%% データ作成
%ノイズあり準備
fileID =fopen('makeyglass_added_uniform_[-0.05 0.05]1200plot300.tex','r');
formatspac='%f';
...
for n=1:1:150    
%リアプノフ指数共通操作
Xria(:,1)=tanh(Win*Test(a+1));%過渡取り除いて活性化201からスタートこれが1番目
...
for t=1:riaSize-1%学習活性化500回繰り返す
    Xria(:,t+1)=tanh(Win*Test(a+(t+1))+W*Xria(:,t));%t+1とtバージョン
    x=Xria(:,t+1);%正しいベクトル
    xp=tanh(Win*Test(a+(t+1))+W*xp);%摂動を与えられた内部状態ベクトル
...
for t=a+1:1:a+SL%201:700
    Tmc(t-a)=Test(t-k);%201番目を入力した時200番目を予測できるか
end
M=M';
Tmc=Tmc';%横ベクトル
...
for t=a+SL+1:1:AL %701から1200 フリーラン予測ではない
    X(:,t)=tanh(Win*Test(t)+W*X(:,t-1));%予測と一緒の時はいらないかも
    outMc=WoutMc'*X(:,t);
    y((t-(a+SL)),k)=outMc;
    outMcbase=WoutMcbase*X(:,t);
    ybase((t-(a+SL)),k)=outMcbase;
end
MCcov=cov(Test(a+SL+1-k:AL-k),y(:,k));%分子部分の共分散計算　Mccovは箱
MCcov=MCcov(1,2);%共分散は2×2の1,2に要素がある
MCcov=MCcov*MCcov;%共分散の2乗
MCk(1,k)=MCcov/(var(Test(a+SL+1:AL))*var(y(:,k)));

MCcov=cov(Test(a+SL+1-k:AL-k),ybase(:,k));%分子部分の共分散計算
MCcov=MCcov(1,2);%共分散は2×2の1,2に要素がある
MCcov=MCcov*MCcov;%共分散の2乗
MCkbase(1,k)=MCcov/(var(Test(a+SL+1:AL))*var(ybase(:,k)));
end
MC=sum(MCk);
MCbase=sum(MCkbase);
%% データ保存
fileID = fopen('LErandam_makey_noise_1200_[-0.05,0.05]SF300.tex','a');
fprintf(fileID,'%f\n',ria);
fclose(fileID);
fileID = fopen('MCrandam_makey_noise_ridge_1200_[-0.05,0.05]SF300.tex','a');
fprintf(fileID,'%f\n',MC);
fclose(fileID);
fileID = fopen('MCrandam_makey_noise_regression_1200_[-0.05,0.05]SF300.tex','a');
fprintf(fileID,'%f\n',MCbase);
fclose(fileID);
%%
end
