...
%各パラメータ
r=0.49+0.01*nr;             %リザーバの固定された絶対値r
b=0.5;                     %リザーバの固定されたバックコネククション絶対値b
v=0.49+0.01*nv;            %入力の重みv
%% 重み作成
...
for t=a+SL+1:AL                                      %出力
  ...
  rmse=sqrt(sum((E(a+SL+1:AL)-preoutbase(1:PL)).^2)/500);
R(t-a-SL,1)=rmse;
end
%%誤差
...
p(n,1)=10^-12;                                   %n番目のニューロンに10^-12を代入　　
...
%% データ保存
fileID = fopen('LEmakey_general_SCR_test2.tex','a');
fprintf(fileID,'%f\n',lya);
fclose(fileID);
fileID = fopen('MCmakey_general_ridge_SCR_test2.tex','a');
fprintf(fileID,'%f\n',MC_ride);
fclose(fileID);
fileID = fopen('MCmakey_general_regression_SCR_test2.tex','a');
fprintf(fileID,'%f\n',MCbase);
fclose(fileID);
fileID = fopen('MCmakey_general_RMSE_SCR_test2.tex','a');
fprintf(fileID,'%f\n',R);
fclose(fileID);
%%
end
