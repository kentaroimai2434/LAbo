%% プログラミング MC 通常マッキーグラス リッジ　一般化　minimum
clear;
clc;
%% データ作成
...
%Win=(rand(resSize,inSize)*2)-1;%[-0.1,0.1]の一様分布(by論文から）
...
%% データ保存
fileID = fopen('LEmakey_1200_SCR_v[0.01:0.01:10.0]_in[exp(-0.5)]_test9.tex','a');
fprintf(fileID,'%f\n',lya);
fclose(fileID);
fileID = fopen('MCmakey_1200_ridge_SCR_v[0.01:0.01:10.0]_in[exp(-0.5)]_test9.tex','a');
fprintf(fileID,'%f\n',MC_ride);
fclose(fileID);
fileID = fopen('MCmakey_1200_regression_SCR_v[0.01:0.01:10.0]_in[exp(-0.5)]_test9.tex','a');
fprintf(fileID,'%f\n',MCbase);
fclose(fileID);
fileID = fopen('MCmakey_1200_RMSE_SCR_v[0.01:0.01:10.0]_in[exp(-0.5)]_test9.tex','a');
fprintf(fileID,'%f\n',rmse);
fclose(fileID);
%%
end
